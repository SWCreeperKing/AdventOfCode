...
file class Day16
{
    [ModifyInput] public static List<bool> ProcessInput(string input) => input.Select(c => c == '1').ToList();

    [Answer("11100110111101110")]
    public static string Part1(List<bool> inp) => CheckSum(DragonCurve(inp)).Select(b => b ? '1' : '0').Join();
    
    [Answer("10001101010000101")]
    public static string Part2(List<bool> inp)
        => CheckSum(DragonCurve(inp, 35651584)).Select(b => b ? '1' : '0').Join();
