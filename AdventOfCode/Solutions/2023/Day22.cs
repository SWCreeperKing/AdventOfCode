...
file class Day22
{
    ...
    public static long Part2(List<Cube> inp)
    {
        ...
        foreach (var cube in inp)
        {
            ...
            foreach (var c in cube.Supporting) check.Enqueue(c, c.ActualZ);
            ...
            while (check.Count > 0)
            {
                ...
                foreach (var candidate in fallingCube.Supporting) check.Enqueue(candidate, candidate.ActualZ);
            }
    ...
    public static void CreateMap(List<Cube> map)
    {
        ...
        {
            Pos pos = new(x, y);
            if (!tetris3d.TryGetValue(pos, out var column)) column = tetris3d[pos] = new Dictionary<int, Cube>();
