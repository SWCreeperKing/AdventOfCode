...
public class Day11
{
    ...
    [Answer(827009909817)] public static long Part2(string[] inp) => Solve(inp, 1000000 - 1);

    public static long Solve(string[] inp, int count = 1)
        => ExpandSpace(inp, [], [], Enumerable.Range(0, inp.Length).ToList(), count).CombinationsUnique()
            .Select((gs, _) => gs.Item1.ManhattanDistance(gs.Item2)).Sum();
        ...
        List<int> rowsWithoutGalaxies, List<int> columnsWithoutGalaxies, int count = 1)
    {
        ...
        return galaxies.Select(xy => (xy.x + columnsWithoutGalaxies.Count(j => j < xy.x) * count,
            xy.y + rowsWithoutGalaxies.Count(j => j < xy.y) * count)).ToList();
    }
}